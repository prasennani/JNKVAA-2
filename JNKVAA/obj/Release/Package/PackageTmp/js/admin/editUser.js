        ...
        success: function (response) {
          //  alert(response.d);
            ...
            if (user[0].ustatus.localeCompare("521") === 0){
                ...
                $('#workingas').val(user[0].designation).change();
                $('#bio').val(user[0].ubio);
...
function changeStatus(status){
    if(userid.length>0){
        $.ajax({
            ...
            contentType: "application/json",
            data: "{ 'uid': '"+userid+"', 'ustatus': '" + status + "'}",
            datatype: "json",
